...
void Object::Update(Camera input_rect)
{
	...
	matrix4 proj = matrix4::Build_scale(vector2(input_rect.right_.x / 2, input_rect.up_.y / 2));
	...
	view_r.value[1][1] = input_rect.up_.x / 2;//vy

	view = view_r * view_t;

	world = matrix4::Build_rotation(transform_.rotation_) * matrix4::Build_scale(transform_.scale_) * matrix4::Build_translation(vector3(transform_.translation_.x, transform_.translation_.y,0.0f));
